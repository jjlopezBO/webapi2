...
namespace cndcAPI.Controllers.ga
{
    [ApiController]
    [Route("[controller]")]
    public class ga_pot_efec_termo : Controller
    {
        private readonly ILogger<ga_pot_efec_termo> _logger;

        public ga_pot_efec_termo(ILogger<ga_pot_efec_termo> logger)
        {
            _logger = logger;
        }

        [HttpGet(Name = "ga_pot_efec_termo")]
        public async Task<IActionResult> GetAsync(string periodo, string subperiodo)
        {
            try
            {
                ...
                {
                    table = await oracle.ExecuteAsync("pkg_apiga.ga_pot_efec_termo", periodoLong, subperiodoLong);
                }
                ...
                var result = Ga_PotEfec_Termo.FromDataTable(table);
