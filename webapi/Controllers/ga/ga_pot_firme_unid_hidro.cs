...
namespace cndcAPI.Controllers.ga
{
    [ApiController]
    [Route("[controller]")]
    public class ga_pot_firme_unid_hidro : Controller
    {
        private readonly ILogger<ga_pot_firme_unid_hidro> _logger;

        public ga_pot_firme_unid_hidro(ILogger<ga_pot_firme_unid_hidro> logger)
        {
            _logger = logger;
        }

        [HttpGet(Name = "ga_pot_firme_unid_hidro")]
        public async Task<IActionResult> GetAsync(string periodo, string subperiodo)
        {
            try
            {
                _logger.LogInformation("Iniciando ga_pot_firme_unid_hidro...");
                ...
                {
                    table = await oracle.ExecuteAsync("pkg_apiga.ga_pot_firme_unid_hidro", periodoLong, subperiodoLong);
                }    
                
            //    string clase = GenerarClaseDesdeDataTable(table, "MiModelo");



                _logger.LogInformation("Consulta de ga_pot_firme_unid_hidro completada con éxito.");

                // Convertir DataTable a DTO
                var result = Ga_PotFirme_Unid_Hidro.FromDataTable(table);

                return Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al ga_pot_firme_unid_hidro.");
