...
namespace cndcAPI.Controllers.ga
{
    [ApiController]
    [Route("[controller]")]
    public class ga_pot_efec_hidro: Controller
    {
        private readonly ILogger<ga_pot_efec_hidro> _logger;

        public ga_pot_efec_hidro(ILogger<ga_pot_efec_hidro> logger)
        {
            _logger = logger;
        }

        [HttpGet(Name = "ga_pot_efec_hidro")]
        public async Task<IActionResult> GetAsync(string periodo, string subperiodo)
        {
            try
            {
                ...
                {
                    table = await oracle.ExecuteAsync("pkg_apiga.ga_potefec_hidro", periodoLong, subperiodoLong);
                }
