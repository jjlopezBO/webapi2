...
namespace cndcAPI.Controllers.ga
{
    [ApiController]
    [Route("[controller]")]
    public class ga_subperiodos : Controller
    {
        private readonly ILogger<WebApiFechas> _logger;

        public ga_subperiodos(ILogger<WebApiFechas> logger)
        {
            _logger = logger;
        }

        [HttpGet(Name = "ga_subperiodos")]
        public async Task<IActionResult> GetAsync()
        {
            try
            {
                _logger.LogInformation("Iniciando WebApiFrecuencia...");
                ...
                {
                    table = await oracle.ExecuteAsync("pkg_apiga.ga_subperiodos");
                }
                ...
                var result = Ga_Subperiodos.FromDataTable(table);
